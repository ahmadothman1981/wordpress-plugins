    ...
    <div className="paying-attention-frontend" style={{ backgroundColor: data.bgColor , textAlign:data.theAlignment}}>
      ...
      <ul>
        {data.answers.map((answer, index) => (
          <li 
            className={(isCorrectDelayed === true && index === data.correctAnswer ? "no-click" : "") + 
                       (isCorrectDelayed === true && index !== data.correctAnswer ? " fade-incorrect" : "")} 
            key={index} 
...
divsToUpdate.forEach(function(div) {
  ...
  root.render(<Quiz data={data}  bgColor/>);

...
class Search {
  constructor() {
    ...
    this.searchResults = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#search-overlay-results');
    this.events();
		 ...
		 <div class="row">
		 	<div class="one-third">
				...
				${results.generalInfo.length ? `<ul class="link-list min-list">
							...
							${item.title}</a> ${item.postType == 'post' ? `by ${item.authorName} ` : ''} </li>`).join('')}</ul>` : '<p>No results found</p>'}
			</div>
			<div class=""one-third">
				<h2 class="search-overlay__section-title">Programs</h2>
				${results.programs.length ? `<ul class="link-list min-list">
											${results.programs.map(item => `<li><a href="${item.permalink}">
											${item.title}</a>  </li>`).join('')}</ul>` : `<p>No results found.<a href="${universityData.root_url}/programs}">View All Our Programs</a></p>`}
				...
				</li>`).join('')}</ul>` : '<p>No results found</p>'}			
			...
			<div class=""one-third">
				...
				${results.campus.length ? `<ul class="link-list min-list">
															...
															${item.title}</a>  </li>`).join('')}</ul>` : `<p>No results found.<a href="${universityData.root_url}/Campuses}">View All Our Campus</a></p>`}
				  ...
				  <h5 class="event-summary__title headline headline--tiny">
					   <a href="${item.permalink}">${item.title}</a></h5>
						     ...
						     <a href="${item.permalink}" class="nu gray">Read more</a></p>
															...
															`).join('')}</ul>` : `<p>No results found.<a href="${universityData.root_url}/events}">View All Our Events</a></p>`}
			</div>
  ...
  keyPressDispatcher(e) {
    if (e.keyCode == 83 && !this.isOverlayState && !jquery__WEBPACK_IMPORTED_MODULE_0___default()("input, textarea").is(':focus')) {
      this.openOverlay();
    }
